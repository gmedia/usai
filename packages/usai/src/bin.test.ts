...
const run = async (env: NodeJS.ProcessEnv) => {
  try {
    const r = await promisify(execFile)(process.execPath, [wrapper, "--version"], {
      env,
      encoding: "utf8",
    });
    return { status: 0, ...r };
...
test("cache dir and release URL follow the environment", () => {
  ...
  assert.match(
    releaseUrl("a.tar.gz", {}),
    new RegExp(
      `^https://github.com/gmedia/usai/releases/download/v${version.replaceAll(".", "\\.")}/a\\.tar\\.gz$`,
    ),
  );
  assert.equal(
    releaseUrl("a.tar.gz", { USAI_RELEASE_BASE: "http://mirror/" }),
    "http://mirror/a.tar.gz",
  );
});
...
test("USAI_BIN is used as is; a same-version usai on PATH wins over a fetch", () => {
  ...
  const explicit = execFileSync(process.execPath, [wrapper, "dev", "--port", "1"], {
    env: { ...process.env, USAI_BIN: fake },
    encoding: "utf8",
  });
...
test("first use fetches the release tarball, verifies its SHA-256, caches the binary; a bad sum is refused", async () => {
  ...
  const server = createServer((req, res) => {
    if (req.url?.endsWith(".sha256"))
      res.end(`${corrupt ? "0".repeat(64) : sum}  ${name}.tar.gz\n`);
...
test("under `pnpm usai` the wrapper is itself on PATH and must not probe itself", () => {
  ...
  const r = spawnSync(join(bin, "usai"), ["--version"], {
    env: {
      ...
      USAI_CACHE_DIR: join(dir, "cache"),
      USAI_RELEASE_BASE: "http://127.0.0.1:9",
    },
    encoding: "utf8",
    timeout: 20_000,
  });
