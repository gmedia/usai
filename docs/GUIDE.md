...
- Migrations: SQL files found by `usai.config.ts` includes and module globs, applied in file-name order by `usai db migrate`, recorded in `usai_migrations`, never run at startup. **The order is global, by file name, across every module — number them project-wide.** A module is organisation, not isolation: one database has one history, a foreign key from one module's table to another's is ordinary, and two per-module histories could not express which comes first. So `0003_decisions.sql` in the `decisions` module runs after `0002_detections.sql` in `detections` because of the numbers, not because of the modules — and a duplicate name across modules is refused rather than ordered arbitrarily. `usai db status` prints the directory each file came from, so the order and its owners are readable at a glance:

  ```console
  migration                          from                     checksum           applied
  0001_detections.sql                src/detections/migrations   …              2026-09-25T…
  0002_decisions.sql                 src/decisions/migrations    …              pending
  ```
 **Each file is its own transaction** — `BEGIN; <advisory lock>; <ledger row>; your SQL; COMMIT` — and three things follow from that. An `ALTER TABLE` and a backfill in the *same file* hold `ACCESS EXCLUSIVE` for the backfill's whole duration, so keep them in separate files. A file that takes minutes blocks writers for minutes while HTTP keeps answering 200 and readiness stays green, so a backfill belongs in a `command()`, not a migration. And `applied_at` is the transaction's *start*, because the ledger row is written before your SQL runs. Several `usai db migrate` at once are safe (the advisory lock plus the ledger's key: each file applies exactly once), but the losers block for the whole run — and the lock is one **per database**, shared by every Usai application in it, so one application's twenty-minute migration blocks another's `db migrate` entirely. Give applications their own database, or sequence their deploys. A file with **`-- usai: no-transaction`** on a line of its own runs outside the transaction, for the statements PostgreSQL refuses inside one — `CREATE INDEX CONCURRENTLY`, `ALTER TYPE … ADD VALUE`, `VACUUM`, `REINDEX CONCURRENTLY`; the ledger row is then not atomic with the work, so **the SQL must be idempotent** (`IF NOT EXISTS`). `usai db status` shows them, names any file edited after it was applied, and `--check` exits non-zero when anything is pending. Changing a live schema — expand/contract, a concurrent index, a batched backfill, the mixed-version queue window — is `docs/runbooks/schema-change.md`. `usai build` copies them into the artifact (`.usai/build/migrations/`), so a production image applies them without a source tree: `usai db migrate --artifact /app/.usai/build` (the scaffold's image: `docker run --rm -e DATABASE_URL=… my-app db migrate --artifact /app/.usai/build`).
...
**A transactional outbox.** `publish` writes on its own connection, so "store the thing and enqueue the work" is two facts: a request that fails after publishing has enqueued work for a row that was never written, and a retried request enqueues it again. Pass the executor your transaction gave you and the queue row joins it — one fact, committed or rolled back together:

```ts
await ctx.resources.db.transaction(async (tx) => {
  const { id } = await tx.one<{ id: number }>("insert into detections (…) values (…) returning id");
  await ctx.queue.publish("evaluate", { detectionId: id }, { tx });   // rolls back with the insert
});
```

The transaction must be on the same resource as the queue — one transaction is one connection to one database — and passing the resource handle instead of the executor is refused rather than quietly committing on its own. It does not make delivery exactly-once: the consumer still needs to be idempotent, because at-least-once is about delivery, not about publishing. What it removes is work enqueued for something that does not exist.
...
**Sizing.** Every open connection is one world for its whole life, out of the same `--max-worlds` budget that serves HTTP requests, at roughly 4 MiB resident each. A few thousand viewers is a `--max-worlds` and a memory decision to take before the feature ships (`docs/runbooks/sizing.md`); past the budget an upgrade is refused with `503` **before** the `101`, which a browser reports as `onerror` and `close 1006` with an empty reason — indistinguishable from a network failure, so a client that wants to back off differently needs the server to say so another way (a `GET` health route before connecting, for instance).
...
- **`fetch`** — outbound HTTP is the `httpClient` resource (§11); the global only explains that. **HTTP is the whole of outbound I/O** (ADR-0024): there is no gRPC client, no raw TCP and no outbound WebSocket. A service that speaks something else is reached through a small process of your own that exposes what you need over HTTP — and that process is usually worth having anyway, because it is where the allow-list goes. A gRPC control plane that can blackhole network prefixes is a good example: the sidecar narrows it to `announce`, `withdraw`, `list` over prefixes you permit, which is a boundary no in-process client would give you.
...
Every flag above has an environment form (`USAI_MAX_WORLDS`, `USAI_DRAIN_GRACE`, `USAI_STATUS_ADDR`, …); `docs/ENVIRONMENT.md` lists them all, with the tuning knobs that are *not* application configuration.
...
is available through `app.logs()` / `app.waitForLog()` (§15) as well as on
the terminal.
...
Per-request logging: there is no access log by design — 2xx and 4xx are counted (`usai_http_responses_total`, per-workload counts in `/_usai/status`), 5xx and lifecycle violations are logged with the workload and world (including a `504 deadline_exceeded`, which used to be a counter and nothing else), and `RUST_LOG='warn,usai_runtime::observability=debug'` adds the per-world trace line (the leading `warn,` keeps the runtime's own WARN and ERROR lines — a filter *replaces* the default rather than adding to it) — `world`, `workload`, `request_id`, `termination`, `duration_ms` and **`cpu_us`**, which is the world's own CPU: `cpu_us` close to `duration_ms` means the handler was *computing*, far below it means it was *waiting* (`docs/runbooks/slow-route.md`). Plain `RUST_LOG=usai_runtime=debug` adds six more lines per request. Put request logging in the proxy in front.
...
Several replicas: **WebSockets and streams need no sticky sessions** — a connection is served by whichever replica the proxy picked, and because fan-out goes through a resource rather than through process memory (§9), an event written on one replica reaches clients connected to any of them, with presence counted cluster-wide. That is the opposite of a Socket.IO deployment, which needs `ip_hash` and a Redis adapter; here there is nothing to configure, and nothing to get wrong. What a proxy *does* need is to allow the upgrade and not to time out a long read (`docs/deploy/compose.production.yaml` has the Caddy block). HTTP and queue consumers share the work without configuration (messages are claimed with `SKIP LOCKED`; migrations serialize on an advisory lock). **A cron schedule runs on every instance that schedules, unless it is declared `exclusive: true`** — then each tick is claimed once in PostgreSQL (`usai_cron_ticks`) and one instance runs it, whichever replica got there first (§6). **A `service` takes the same option**, for work that is a loop rather than a tick: `service("reconcile", { exclusive: true, … })` claims a lease in `usai_service_leases` and only the holder starts the world, while the others report `waiting` in `/_usai/status` — healthy and ready, neither stopped nor failed. The holder renews every third of `leaseMs` (30 s by default), so a replica that dies without draining is replaced within about that long, and one that drains gives the lease up so the next starts at once. Losing the lease **stops** the world rather than cancelling it, so the handler sees `ctx.signal` and can finish what it holds — which is the point of the option: a reconciler comparing desired and actual state should have one writer, and two replicas for availability. Schedules without it need the scheduler on one replica: start the others with `usai run --no-cron` (`USAI_NO_CRON=1`); `--no-queue` likewise dedicates replicas. `SUPPORTED.md` has the topology table.
