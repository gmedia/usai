...
- Any Standard Schema library works (Zod 4, Valibot, ArkType …). When the library can describe itself as JSON Schema (Zod 4 does), invalid input is rejected **before a world exists**; otherwise it is validated inside the world. The handler always receives the library's own output: a schema that only checks (no defaults, coercion, transforms, `catch`) is validated once, at the boundary, and the world merely strips undeclared keys the way a plain `z.object` does; a schema that changes the value is parsed again in the world so `.default()`, `.transform()` and friends behave exactly as the library defines them.
