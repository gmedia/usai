...
- **Uploads.** A file arrives on an `http.raw` route as the exact bytes (`ctx.request.bytes()`); `multipart.parse(bytes, ctx.headers["content-type"])` splits a `multipart/form-data` body into `fields` and `files` (`{ name, filename, contentType, data }`), and a `Uint8Array` goes straight into a `bytea` column. The body bound applies (`USAI_MAX_BODY_BYTES`); anything larger than a form attachment is better uploaded by the client straight to object storage with a presigned URL the application mints (HMAC-SHA256 is in `crypto.subtle`). A raw route cannot declare its request content type; the document lists it as opaque.
