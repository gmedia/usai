...
Current phase (`docs/ROADMAP.md`): **P5 done, P6 done (72 h soak passed), P7 in progress, P8 c=1 done, P8E done; the sweep is in and P9's scope is decided (ADR-0019).** Depth, not breadth.
...
1. **Threat model verified, not just written** (2026-09-23, `docs/measurements/2026-09-23-threat-verification.md`): `scripts/qualification/threat/run.sh` checks every claim in `docs/THREAT-MODEL.md` that is visible from outside the process — 42 probes (three added 2026-09-24 with `--server-timing`: an ordinary response carries no `Server-Timing` and no `x-usai-*`, and does return `x-request-id`), all passing, now part of the release gate. The two bugs it found were in the instrument (a signature probe that read a file `keygen` never writes; a “no world was created” check against a gauge that was still zero). Not covered there, by design: the socket idle timeout (connection campaign), admission refusal (hardening suite), the routing gate (lifecycle suite), connection quarantine (postgres suite).
1. ~~Propose the Wasmtime pagemap-reset patch upstream~~ — **merged into `bytecodealliance/wasmtime` main on 2026-09-23** (#14357, `a2e2d86`): review answered the same day, the maintainer called the fix and the speed-up reasonable and classed the paged-out-page hole as a bugfix. What is left is mechanical: the first wasmtime release after the merge (v49.0.0 predates it) replaces the vendor — re-vendor that version, drop `vendor/wasmtime` and the patch, keep the freshness tests (`docs/upstream/wasmtime-pagemap-reset.md`); the vendor stays until a release carries it. ~~The eval-based invoke/outcome/pending floor~~ — gone (ADR-0018; P8 c=1 phase done, `docs/measurements/2026-09-19-p8-parity.md`).
1. ~~Propose the idle-decommit API upstream~~ — **merged into
   `bytecodealliance/wasmtime` main on 2026-09-26** (#14419, `65c5190`):
   `Engine::release_idle_pool_memory()`, approved with four comments, all of
   them about the comments. It answers #14413, which exists because the first
   attempt (#14399) was closed under the Bytecode Alliance AI tool policy —
   an assistant drafted and filed it with nobody having read it, which is the
   pattern that policy exists to stop. The second attempt was written and
   posted by a person, half the length, and asked a question instead of
   presenting a design. **Two upstream contributions, two different lessons**:
   #14357 showed a small change with tests gets read in a day; #14399 showed
   what happens when the human in the loop is missing. A third lesson
   arrived with the measurement: the contribution was right and the *reason*
   we wanted it was wrong (`docs/measurements/2026-09-26-idle-decommit.md`).
   The plateau ADR-0019 described had already been closed by a fix from
   another campaign, and nobody noticed because the two were written a week
   apart and never measured together.
2. **Running now on VM 47** (2026-09-23/24): the **24 h bounded-dataset soak**
   (`docs/measurements/2026-09-24-bounded-soak.md`), and the comparator floors
