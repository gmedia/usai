...
| P9 lever 2 upstream | ✓ merged, ours unbuilt | **[bytecodealliance/wasmtime#14419](https://github.com/bytecodealliance/wasmtime/pull/14419) merged into `main` 2026-09-26** (`65c5190`): `Engine::release_idle_pool_memory()`, the second of ADR-0019's three P9 levers. It answers [#14413](https://github.com/bytecodealliance/wasmtime/issues/14413), filed by the maintainer of this repository after a first attempt (#14399) was closed under the Bytecode Alliance AI tool policy — the second is the one written and posted by a person, which is the whole difference. Measured before it went up: 64 slots at `keep_resident` 8 MiB give back **256 MiB** of RSS (266.9 → 10.9 MiB) and the first instantiation afterwards pays ≈2 ms re-faulting (`docs/upstream/idle-decommit-measurement/`). **Nothing here changes yet**: the runtime is on wasmtime 48, and the method arrives only in a release whose *file* carries it — the same rule the pagemap vendor learned. Our side is one call from `idle.rs`, a `usai_pool_memory_released_total` counter and the P8E before/after re-run. One claim in the issue was wrong and is corrected in `docs/upstream/wasmtime-idle-decommit.md`: the kept region holds the module's image, not zeros; the conclusion survives because `RestoreOriginalMapping` puts those same contents back |
...
1. ~~Propose the idle-decommit API upstream~~ — **merged into
   ...
   what happens when the human in the loop is missing.
