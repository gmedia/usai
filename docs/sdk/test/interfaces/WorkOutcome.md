...
| <a id="violations"></a> `violations` | \{ `code`: `string`; `message`: `string`; \}[] | The world's lifecycle violations, with the message that explains each one. The response-side [TestResponse.violations](TestResponse.md#violations) is `string[]` — codes only — because a response header is all a client gets. |
