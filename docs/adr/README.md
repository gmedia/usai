...
| [0020](0020-artifact-signing-and-what-the-format-promises.md) | One Ed25519 signature covers every file the runtime reads (the native image included), verified before the listener binds; the manifest is the compatibility surface and the rest of the directory is a build output, not an interchange format | accepted (v0) | ADR-0005 follow-up |
| [0021](0021-cpu-is-accounted-not-scheduled.md) | CPU is measured per workload and capped with admission and deadlines; no scheduler, quota or priority in v0, because preemption of guest execution is the machinery that would make state survive a world | accepted (v0) | ADR-0008, Q17, Q18 |
