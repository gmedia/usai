...
| `usai_pool_memory_resident_unused_bytes` | gauge | — | What the substrate holds resident for slots that are **warm but unused**: the plateau. It is `USAI_WASM_KEEP_RESIDENT` (8 MiB) × slots touched and not yet reclaimed, so after a burst it is most of the difference between idle RSS and burst RSS. Wasm engine only |
| `usai_pool_memory_released_bytes_total` | counter | — | Bytes handed back after a quiet period (`USAI_IDLE_DECOMMIT_MS`, default 30 s). Rising means the plateau is being reclaimed; the first world in each released slot pays ≈2 ms re-faulting for it |
| `usai_pool_memory_releases_total` | counter | — | How many times the release ran. **`0` with a large `..._resident_unused_bytes` is not a fault**: it means the plateau never reached `USAI_IDLE_DECOMMIT_FLOOR` (64 MiB), or the process has not been quiet that long — which is the normal reading on a service that is never idle |
...
| Plateau never reclaimed | `usai_pool_memory_resident_unused_bytes > 256e6 and increase(usai_pool_memory_releases_total[1h]) == 0` | the box is holding a quarter of a gigabyte for slots nothing is using and the policy has not fired in an hour. Either the instance is never quiet for `USAI_IDLE_DECOMMIT_MS` (normal under steady load — then size for the plateau) or the release is off (`USAI_IDLE_DECOMMIT_MS=0`) |
