...
| `usai_pool_memory_resident_unused_bytes` | gauge | — | What the substrate holds resident for slots that are **warm but unused**. Read it against RSS rather than as RSS: measured at 12 MiB after a c=32 burst that raised RSS by 66 MiB, because this pool re-picks freed slots instead of accumulating them and the rest of the growth is per-slot copy-on-write image pages (`measurements/2026-09-26-idle-decommit.md`). Wasm engine only |
| `usai_pool_memory_released_bytes_total` | counter | — | Bytes handed back after a quiet period. Stays 0 unless `USAI_IDLE_DECOMMIT_MS` is set, which is **off by default and measured to reclaim almost nothing** (`measurements/2026-09-26-idle-decommit.md`) |
| `usai_pool_memory_releases_total` | counter | — | How many times the release ran; `0` is the default reading, since the policy is off |
