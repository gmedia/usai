...
// same SQL, the same error envelope. PORT, DATABASE_URL, POOL_MAX; CLUSTER=<n>
// forks n workers sharing the port (node:cluster) — the fair comparator for
// c > 1 on several cores, since one Node process is one thread.
...
const workers = Number(process.env.CLUSTER ?? 1);
