...
console.log(
  "Performance scoreboard (each server does the same validated work; see BENCHMARKS.md).\n",
);
for (const [cls, title] of Object.entries(classes)) {
  ...
  console.log(
    "| server | c | req/s | p50 ms | p95 ms | p99 ms | CPU ms/req | RSS MiB | 4xx | 5xx | errors |",
  );
  console.log("|---|---|---|---|---|---|---|---|---|---|---|");
  for (const r of rows)
    console.log(
