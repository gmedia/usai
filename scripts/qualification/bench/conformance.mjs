...
if (!base) {
  console.error("usage: conformance.mjs <base>");
  process.exit(2);
}
...
const quote = {
  ...
  reference: ref,
  tags: ["a"],
  items: [{ sku: "A1", quantity: 2, unitCents: 1500 }],
};
...
function sameShape(expected, actual) {
  ...
  if (Array.isArray(expected)) {
    if (!Array.isArray(actual) || actual.length !== expected.length)
      return `array ${JSON.stringify(actual)}`;
    for (let i = 0; i < expected.length; i++) {
      const r = sameShape(expected[i], actual[i]);
      if (r !== true) return r;
    }
    return true;
  }
  if (actual === null || typeof actual !== "object")
    return `expected object, got ${JSON.stringify(actual)}`;
  for (const k of Object.keys(expected)) {
    if (k === "message" && typeof actual[k] === "string") continue;
    const r = sameShape(expected[k], actual[k]);
    if (r !== true) return `${k}: ${r}`;
  }
  return true;
}
...
await check(
  "A hello 400 (too long)",
  "GET",
  "/hello/" + "x".repeat(41),
  {},
  400,
  (b) => b?.error?.code === "validation_failed" || `code ${b?.error?.code}`,
);
await check("B quote 201", "POST", "/orders/quote", json(quote), 201, {
  reference: ref,
  currency: "IDR",
  subtotalCents: 3000,
  taxCents: 330,
...
await check(
  "B quote 400 (bad email)",
  "POST",
  "/orders/quote",
  json({ ...quote, email: "nope" }),
  400,
  (b) => b?.error?.code === "validation_failed" || `code ${b?.error?.code}`,
);
await check(
  "B quote 400 (no items)",
  "POST",
  "/orders/quote",
  json({ ...quote, items: [] }),
  400,
...
await check("C user 200", "GET", "/users/42", {}, 200, {
  id: 42,
  name: "user-42",
...
await check(
  ...
  "POST",
  "/users",
  json({ name: "Conf", email: `conf-${suffix}@example.test` }),
  201,
  (b) => (typeof b?.id === "number" && b.name === "Conf") || `body ${JSON.stringify(b)}`,
);
await check(
  "D create 409",
  "POST",
...
await check(
  ...
  (b) =>
    (b?.orderId === orderId &&
      b.paid === true &&
      typeof b.paymentId === "number" &&
      typeof b.amountCents === "number") ||
    `body ${JSON.stringify(b)}`,
);
await check(
  "E pay 409 (already paid)",
  "POST",
  `/orders/${orderId}/pay`,
  {},
  409,
...
await check("F me 200", "GET", "/me", { headers: { "x-api-key": "key-5" } }, 200, {
  id: 5,
  name: "user-5",
...
await check(
  ...
  {},
  200,
  (b) => typeof b?.count === "number" || `body ${JSON.stringify(b)}`,
);
