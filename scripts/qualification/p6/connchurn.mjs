...
async function sseOnce(events = 2, everyMs = 200) {
  ...
  const res = await fetch(`${base}/invoices/live?everyMs=${everyMs}`, {
    headers,
    signal: ctl.signal,
  });
  if (res.status !== 200) {
...
function wsOnce() {
  return new Promise((ok, bad) => {
    ...
    const timer = setTimeout(() => {
      s.close();
      bad(new Error(`ws timeout after ${got.length} messages`));
    }, 10_000);
    s.addEventListener("open", () => s.send(JSON.stringify({ type: "counts" })));
    s.addEventListener("message", (e) => {
      got.push(JSON.parse(e.data).type);
      if (got.length === 2) {
        clearTimeout(timer);
        s.close(1000, "done");
        ok(performance.now() - t);
      }
    });
    s.addEventListener("error", () => {
      clearTimeout(timer);
      bad(new Error("ws error"));
    });
    s.addEventListener("close", (e) => {
      if (got.length < 2) {
        clearTimeout(timer);
        bad(new Error(`ws closed ${e.code} ${e.reason}`));
      }
    });
  });
}
...
if (phase === "cycles") {
  ...
  const worker = async () => {
    while (next < n) {
      next++;
      try {
        out.sse.ms.push(await sseOnce());
        out.sse.ok++;
      } catch (e) {
        out.sse.errors[e.message] = (out.sse.errors[e.message] ?? 0) + 1;
      }
      try {
        out.ws.ms.push(await wsOnce());
        out.ws.ok++;
      } catch (e) {
        out.ws.errors[e.message] = (out.ws.errors[e.message] ?? 0) + 1;
      }
    }
  };
  ...
  console.log(
    JSON.stringify({
      ...
      sse: {
        ok: out.sse.ok,
        errors: out.sse.errors,
        p50: pct(out.sse.ms, 0.5),
        p99: pct(out.sse.ms, 0.99),
      },
      ws: {
        ok: out.ws.ok,
        errors: out.ws.errors,
        p50: pct(out.ws.ms, 0.5),
        p99: pct(out.ws.ms, 0.99),
      },
    }),
  );
} else if (phase === "hold") {
  const count = Number(a1 ?? 100),
    seconds = Number(a2 ?? 60);
  const closes = [],
    probes = [];
  let events = 0,
    messages = 0,
  ...
  for (let i = 0; i < count / 2; i++) {
    holders.push(
      (async () => {
        ...
        try {
          const res = await fetch(`${base}/invoices/live?everyMs=1000`, {
            headers,
            signal: ctl.signal,
          });
          if (res.status !== 200) {
          ...
          for (;;) {
            const { value, done } = await reader.read();
            if (done) {
              closes.push({ kind: "sse", at: sec(), how: "ended" });
              return;
            }
            events += new TextDecoder().decode(value).split("\n\n").length - 1;
          }
        } catch (e) {
          if (e.name !== "AbortError") closes.push({ kind: "sse", at: sec(), how: e.message });
        }
      })(),
    );
    holders.push(
      new Promise((done) => {
        ...
        const ping = setInterval(() => {
          if (s.readyState === 1) s.send(JSON.stringify({ type: "counts" }));
        }, 1000);
        const end = setTimeout(() => {
          clearInterval(ping);
          if (s.readyState <= 1) s.close(1000, "hold over");
        }, seconds * 1000);
        ...
        s.addEventListener("close", (e) => {
          clearInterval(ping);
          clearTimeout(end);
          if (e.reason !== "hold over")
            closes.push({ kind: "ws", at: sec(), code: e.code, reason: e.reason });
          done();
        });
      }),
    );
  ...
  const probe = setInterval(async () => {
    ...
    try {
      await sseOnce(1, 200);
      await wsOnce();
      probes.push({ at: t, ok: true });
    } catch (e) {
      probes.push({ at: t, ok: false, error: e.message });
    }
  }, 1000);
  ...
  for (const c of closes) {
    const k = `${c.kind}:${c.code ?? c.how ?? ""}:${c.reason ?? ""}`;
    byReason[k] = (byReason[k] ?? 0) + 1;
  }
  const firstClose = closes.length ? Math.min(...closes.map((c) => c.at)) : null,
    lastClose = closes.length ? Math.max(...closes.map((c) => c.at)) : null;
  ...
  console.log(
    JSON.stringify({
      ...
      events,
      messages,
      serverCloses: closes.length,
      byReason,
      firstCloseAt: firstClose,
      lastCloseAt: lastClose,
      probes: probes.length,
      failedProbes: failedProbes.length,
      failedProbeSeconds: failedProbes.map((p) => p.at),
      probeErrors: [...new Set(failedProbes.map((p) => p.error))],
    }),
  );
  process.exit(0);
} else if (phase === "slow") {
  ...
  await new Promise((done) => {
    ...
    const t = Date.now();
    const timer = setTimeout(() => {
      console.log(JSON.stringify({ phase, closed: false, waited: seconds }));
      s.close();
      done();
    }, seconds * 1000);
    s.addEventListener("close", (e) => {
      ...
      console.log(
        JSON.stringify({
          ...
          code: e.code,
          reason: e.reason,
          afterSeconds: +((Date.now() - t) / 1000).toFixed(1),
        }),
      );
      done();
    });
