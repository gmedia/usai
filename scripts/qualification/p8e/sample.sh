...
read_stat() { # pid -> "minflt majflt utime+stime"
  local rest
  ...
  echo "$rest" | awk '{print $8, $10, $12+$13}'
}
...
while :; do
  ...
  for p in "${procs[@]}"; do
    ...
    read -r minflt majflt ticks < <(read_stat "$pid" || echo "0 0 0")
    entries="$entries{\"label\":\"$label\",\"pid\":$pid,\"rssKib\":${rss:-0},\"pssKib\":${pss:-0},\"vmKib\":${vm:-0},\"threads\":${thr:-0},\"fds\":$fds,\"minflt\":$minflt,\"majflt\":$majflt,\"cpuTicks\":$ticks},"
  done
  entries="${entries%,}"
