...
const clients = Number(clientsArg ?? 8), seconds = Number(secondsArg ?? 60);
...
async function one(i, n) {
  ...
  try {
    ...
    const res = kind === "create"
      ? await fetch(`${base}/invoices`, { method: "POST", headers, body: JSON.stringify({ customer: `load-${i}`, currency: "USD", dueDate: "2030-01-01", items: [{ description: "x", quantity: 1, unitCents: 100 }] }), signal: AbortSignal.timeout(10_000) })
      : await fetch(`${base}/invoices?limit=5`, { headers, signal: AbortSignal.timeout(10_000) });
    await res.arrayBuffer();
    if (res.status < 300) window.ok++; else if (res.status === 503) window.s503++; else if (res.status < 500) window.s4xx++; else window.s5xx++;
  } catch { window.errors++; }
  window.latencies.push(performance.now() - t);
}
async function client(i) { let n = 0; while (!stop) await one(i, n++); }
...
const tick = setInterval(() => {
  const w = window; window = { ok: 0, s4xx: 0, s5xx: 0, s503: 0, errors: 0, latencies: [] };
  const line = JSON.stringify({ t: Math.round((Date.now() - started) / 1000), ok: w.ok, s4xx: w.s4xx, s5xx: w.s5xx, s503: w.s503, errors: w.errors, p50: pct(w.latencies, 0.5), p99: pct(w.latencies, 0.99) });
  console.log(line); if (out) appendFileSync(out, line + "\n");
}, 1000);
const runs = Array.from({ length: clients }, (_, i) => client(i));
setTimeout(() => { stop = true; }, seconds * 1000);
await Promise.all(runs); clearInterval(tick);
