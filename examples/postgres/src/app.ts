...
import { defineApp, http, command, postgres, errors, env } from "@sakaladev/usai";
import { z } from "zod";
...
export const createUser = http.post(
  "/users",
  { body: NewUser, response: { 201: User }, resources: [db] },
  async (ctx) => {
    try {
      const row = await ctx.resources.main.one<z.infer<typeof User>>(
...
export const getUser = http.get(
  "/users/:id",
  { params: Id, response: { 200: User }, resources: [db] },
  async (ctx) => {
    const row = await ctx.resources.main.one<z.infer<typeof User>>(
...
export const listUsers = http.get(
  "/users",
  { response: { 200: z.array(User) }, resources: [db] },
  async (ctx) => {
    return ctx.resources.main.query<z.infer<typeof User>>(
