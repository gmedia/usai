...
async function sink() {
  ...
  const server = createServer((req: IncomingMessage, res) => {
    ...
    req.on("end", () => {
      ...
      received.push({
        event: String(req.headers["x-invoicing-event"]),
        signature: String(req.headers["x-invoicing-signature"]),
        body,
      });
...
test("invoicing: tenants, sessions, transactional invoices, pagination, signed retried webhooks, cron, command", {
  ...
  try {
    ...
    const weak = await app.http.post("/signup", {
      body: { tenant: acme, email: "ada@acme.test", password: "short" },
    });
    assert.equal(weak.status, 400);
    const signup = await app.http.post("/signup", {
      body: { tenant: acme, email: "ada@acme.test", password: "correct horse battery staple" },
    });
    assert.equal(signup.status, 201, signup.text);
    const { token } = signup.body as { token: string };
    const duplicate = await app.http.post("/signup", {
      body: { tenant: acme, email: "bob@acme.test", password: "another long password" },
    });
    ...
    assert.equal(
      (await app.http.get("/me", { headers: { authorization: "Bearer nope" } })).status,
      401,
      "unknown token",
    );
    ...
    const login = await app.http.post("/login", {
      body: { tenant: acme, email: "ADA@acme.test", password: "correct horse battery staple" },
    });
    assert.equal(login.status, 200, login.text);
    const wrong = await app.http.post("/login", {
      body: { tenant: acme, email: "ada@acme.test", password: "not the password, sorry" },
    });
    ...
    assert.equal(
      (await app.http.put("/tenant/webhook", { headers: auth, body: { url: hooks.url, secret } }))
        .status,
      200,
    );

    // Invoices: created in one transaction with their items.
    const created = await app.http.post("/invoices", {
      headers: auth,
      body: {
        customer: "Globex",
        currency: "USD",
        dueDate: "2020-01-01",
        items: [
          { description: "Design", quantity: 2, unitCents: 5000 },
          { description: "Hosting", quantity: 1, unitCents: 1999 },
        ],
      },
    });
    assert.equal(created.status, 201, created.text);
    const invoice = created.body as {
      id: string;
      number: number;
      totalCents: number;
      status: string;
    ...
    const invalid = await app.http.post("/invoices", {
      headers: auth,
      body: { customer: "", currency: "GBP", dueDate: "soon", items: [] },
    });
    assert.equal(invalid.status, 400);
    for (let i = 0; i < 4; i++) {
      const r = await app.http.post("/invoices", {
        headers: auth,
        body: {
          customer: `C${i}`,
          currency: "EUR",
          dueDate: "2030-01-01",
          items: [{ description: "x", quantity: 1, unitCents: 100 * (i + 1) }],
        },
      });
    ...
    assert.deepEqual(
      p1.items.map((i) => i.number),
      [5, 4],
    );
    assert.ok(p1.nextCursor);
    const page2 = await app.http.get("/invoices", {
      headers: auth,
      query: { limit: "2", cursor: p1.nextCursor! },
    });
    const p2 = page2.body as { items: Array<{ number: number }>; nextCursor: string | null };
    assert.deepEqual(
      p2.items.map((i) => i.number),
      [3, 2],
    );
    const page3 = await app.http.get("/invoices", {
      headers: auth,
      query: { limit: "2", cursor: p2.nextCursor! },
    });
    const p3 = page3.body as { items: Array<{ number: number }>; nextCursor: string | null };
    assert.deepEqual(
      p3.items.map((i) => i.number),
      [1],
    );
    ...
    assert.equal(
      (again.body as { error: { details: { status: string } } }).error.details.status,
      "issued",
    );
    ...
    const other = await app.http.post("/signup", {
      body: {
        tenant: umbrella,
        email: "eve@umbrella.test",
        password: "a completely different secret",
      },
    });
    const otherAuth = { authorization: `Bearer ${(other.body as { token: string }).token}` };
    assert.equal(
      (await app.http.get(`/invoices/${invoice.id}`, { headers: otherAuth })).status,
      404,
      "tenants do not see each other",
    );
    assert.equal(
      ((await app.http.get("/invoices", { headers: otherAuth })).body as { items: unknown[] }).items
        .length,
      0,
    );
    ...
    const payload = JSON.parse(hit.body) as {
      attempt: number;
      invoice: { number: number; status: string };
    };
    ...
    assert.ok(
      ...
      "ours (plus the seeded demo tenant's on a fresh database; no webhook there)",
    );
    assert.ok(
      await waitFor(() => hooks.received.some((r) => r.event === "invoice.overdue"), 20_000),
      "overdue webhook",
    );
    ...
    const ws = new WebSocket(`${app.url.replace(/^http/, "ws")}/invoices/socket`, {
      headers: auth,
    } as never);
    ...
    const demo = await app.http.post("/login", {
      body: { tenant: "demo", email: "owner@demo.test", password: "demo-demo-demo-demo" },
    });
