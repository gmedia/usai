...
export default seeder({ resources: [db] }, async (ctx) => {
  ...
  await sql.transaction(async (tx) => {
    const tenant = await tx.one<{ id: string }>(`insert into tenants (slug, name, invoice_seq) values ('demo', 'Demo Co', 3) returning id`);
    await tx.execute(`insert into users (tenant_id, email, password_hash) values ($1, 'owner@demo.test', $2)`, [tenant!.id, hash]);
    const rows = [
    ...
    for (const [number, customer, currency, status, total, due, issuedAt] of rows) {
      ...
      await tx.execute(`insert into invoice_items (invoice_id, description, quantity, unit_cents) values ($1, 'Services', 1, $2)`, [inv!.id, total]);
    }
  });
  return { seeded: true };
});
