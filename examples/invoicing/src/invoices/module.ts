...
async function publish(
  ctx: { queue: { publish(topic: string, message: unknown): Promise<unknown> } },
  tenantId: string,
  invoiceId: string,
  event: InvoiceEvent,
) {
...
export const create = http.post(
  "/invoices",
  {
    ...
    description:
      "The invoice starts as a draft; issue it to make it payable and notify the tenant's webhook.",
    auth: session,
    body: NewInvoice,
    response: { 201: InvoiceWithItems },
    resources: [db],
  },
  async (ctx) => {
    ...
    const id = await sql(ctx).transaction(async (tx) => {
      ...
      for (const item of ctx.body.items) {
        await tx.execute(
          ...
          [row!.id, item.description, item.quantity, item.unitCents],
        );
      }
      return row!.id;
    });
...
async function load(db: SqlExecutor, tenantId: string, id: string) {
  const invoice = await db.one<InvoiceRow>(
  ...
  const items = await db.query<{
    ...
    `select id::int as id, description, quantity, unit_cents::int as "unitCents" from invoice_items where invoice_id = $1 order by id`,
    [id],
  );
...
export const list = http.get(
  "/invoices",
  {
    ...
    description: "Newest first, cursor-paginated; filter by status.",
    auth: session,
    query: ListQuery,
    response: { 200: Page },
    resources: [db],
  ...
  async (ctx) => {
    ...
    return {
      items,
      nextCursor:
        rows.length > ctx.query.limit && last ? btoa(`${last.createdAt}|${last.id}`) : null,
    };
  },
);
...
async function transition(
  ...
  if (!row) {
    ...
    throw errors.conflict(`invoice is ${exists.status}; cannot make it ${to}`, {
...
export const issue = publishes(
  http.post(
    "/invoices/:id/issue",
    {
      summary: "Issue a draft",
      description: "Draft → issued. Publishes invoice.issued to the tenant's webhook.",
      auth: session,
      params: Id,
...
export const pay = publishes(
  http.post(
    "/invoices/:id/pay",
    {
      summary: "Mark an invoice paid",
      description: "Issued or overdue → paid. Publishes invoice.paid.",
      auth: session,
      params: Id,
      response: { 200: Invoice },
      errors: [
