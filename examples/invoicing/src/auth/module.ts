...
const Credentials = z.object({ email: z.string().email().max(200), password: z.string().min(12).max(200) });
const Signup = Credentials.extend({ tenant: z.string().regex(/^[a-z0-9-]{2,40}$/), name: z.string().min(1).max(120).optional() });
const Session = z.object({ token: z.string(), expiresAt: z.string() });
const Me = z.object({ userId: z.string(), tenantId: z.string(), email: z.string(), tenant: z.string() });
...
async function openSession(ctx: { resources: Record<string, unknown>; env: Record<string, unknown> }, userId: string) {
  const token = hex(crypto.getRandomValues(new Uint8Array(32)));
  const ttlHours = typeof ctx.env["SESSION_TTL_HOURS"] === "number" ? ctx.env["SESSION_TTL_HOURS"] : 24 * 7;
...
export const signup = http.post("/signup", { body: Signup, response: { 201: Session }, errors: [{ code: "conflict", status: 409 }], resources: [db] }, async (ctx) => {
  ...
  const userId = await sql(ctx).transaction(async (tx) => {
    ...
    const tenant = await tx.one<{ id: string }>(`insert into tenants (slug, name) values ($1, $2) returning id`, [ctx.body.tenant, ctx.body.name ?? ctx.body.tenant]);
    const user = await tx.one<{ id: string }>(`insert into users (tenant_id, email, password_hash) values ($1, $2, $3) returning id`, [tenant!.id, ctx.body.email.toLowerCase(), hash]);
    return user!.id;
  });
  return http.created(await openSession(ctx, userId));
});

export const login = http.post("/login", { body: Credentials.extend({ tenant: z.string() }), response: { 200: Session }, errors: [{ code: "unauthorized", status: 401 }], resources: [db] }, async (ctx) => {
  ...
  const ok = user ? await password.verify(ctx.body.password, user.passwordHash) : await password.verify(ctx.body.password, await password.hash("unused"));
...
export const me = http.get("/me", { auth: session, response: { 200: Me }, resources: [db] }, async (ctx) => {
  const tenant = await sql(ctx).one<{ slug: string }>(`select slug from tenants where id = $1`, [ctx.auth.tenantId]);
  return { ...ctx.auth, tenant: tenant!.slug };
});

export const setWebhook = http.put("/tenant/webhook", { auth: session, body: Webhook, resources: [db] }, async (ctx) => {
  await sql(ctx).execute(`update tenants set webhook_url = $1, webhook_secret = $2 where id = $3`, [ctx.body.url, ctx.body.secret, ctx.auth.tenantId]);
